...
/// Calculate spectral entropy from a series of Thornfiddle Path values
pub fn calculate_spectral_entropy(thornfiddle_paths: &[f64]) -> f64 {
    ...
    // Pad series to a power of 2 for FFT efficiency
    let mut padded_series = thornfiddle_paths.to_vec();
    let original_len = padded_series.len();
    ...
    // Create FFT planner and forward FFT
    ...
    // Calculate power spectrum (only use first half due to symmetry)
    ...
    let max_entropy = (fft_size as f64 / 2.0 - 1.0).log2();
    if max_entropy > 0.0 {
        entropy / max_entropy
    } else {
        entropy
